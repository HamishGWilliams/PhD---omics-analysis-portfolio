...
for (experiment in experiments){ # start loop for each experiment
  for (context in contexts){ # start looping through each context too...
    if (experiment == "exp1") { # used to help generate correct list of files and data
      ...
      # build the sample.id
      sample.id = list("3","9","13","15","16","17","22","27") # remove the 1st and 6th values when removing outlier
      
      # build the treatment
      treatment = c(1, 0, 0, 1, 0, 0, 1, 1) # remove the 1st and 6th values when removing outlier
      ...
      file.list = list(paste0("../../Data2/Trimmed/Sample_6-6_D/meth_", context, "_cov_reads"),
                       ...
                       paste0("../../Data2/Trimmed/Sample_8-8_D/meth_", context, "_cov_reads"),
                       ...
                       paste0("../../Data2/Trimmed/Sample_32-18_redo_D/meth_", context, "_cov_reads"),
    ...
    tryCatch({
      filename <- paste0("raw_methylation_plots_act_", context, "_", experiment, ".png")
    ...
    tryCatch({
      filename <- paste0("raw_coverage_plots_act_", context, "_",  experiment, ".png")
    ...
    tryCatch({
    filename <- paste0("clustering_plot", context, "_", experiment, ".png")
    ...
    tryCatch({
      ...
      filename <- paste0("PCA_", 1, "-", 2, "_plot_", context, "_", experiment, ".png")
    ...
    tryCatch({
      ...
      file.name = paste0("act_diff_df_ann", "_", experiment, "_", context, ".txt")
    ...
    data <- act_diff_data
      ...
      volcano_plot <- ggplot(data = data, aes(x = meth.diff, y = -log10(p_fdr), col = diffmeth)) +
        ...
        ggtitle(paste("Differential Methylation of", context, "in", experiment)) + 
      ...
      filename <- paste0("volcano_plot_", experiment, "_", context, ".png")
