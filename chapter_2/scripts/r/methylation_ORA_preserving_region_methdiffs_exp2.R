# Load DMR data
dmr_data <- read.table(
  "C:/Users/r02hw22/OneDrive/Documents/PhD/PhD---omics-analysis-portfolio/chapter_2/data/temp_storage/exp2_DMRs_gene_id_matched.txt",
  header = TRUE,
  sep = "\t",
...
ora_results_exp2_by_region <- list()
...
for (target_region in region_types) {
  ...
  # Save to list
  ora_results_exp2_by_region[[target_region]] <- all_results
  ...
  # Save to CSV
  output_file <- paste0("ORA_results_", target_region, "_exp2.csv")
...
ORA_results_exp2_combined <- bind_rows(
  ora_results_exp2_by_region,
  .id = "region_from_list"
)
...
ORA_results_exp2_plot_df <- ORA_results_exp2_combined %>%
  distinct(
    ID,
    Description,
    GO,
...
ORA_results_exp2_plot_df_sig <- ORA_results_exp2_plot_df %>%
  filter(!is.na(p.adjust), p.adjust <= 0.1)

# If no significant terms exist, fall back to all terms
if (nrow(ORA_results_exp2_plot_df_sig) > 0) {
  ORA_plot_data <- ORA_results_exp2_plot_df_sig
} else {
  ORA_plot_data <- ORA_results_exp2_plot_df
}
...
# If no significant terms exist, fall back to all terms
if (nrow(ORA_results_exp2_plot_df_sig) > 0) {
  ORA_plot_base <- ORA_results_exp2_plot_df_sig
} else {
  ORA_plot_base <- ORA_results_exp2_plot_df
}
...
ORA_methdiff_dot_df_fixed <- ORA_results_exp2_combined %>%
  mutate(
...
ORA_exp2_methdiff_dotplot_fixed <- ggplot(
...
plot(ORA_exp2_methdiff_dotplot_fixed)
ggsave("ORA_exp2_methdiff_dotplot_fixed.png", ORA_exp2_methdiff_dotplot_fixed, width = 24, height = 6, units= "cm", dpi = 600)

# plot together ----
library(patchwork)

combined_methdiff_dotplot <- 
  (ORA_exp1_methdiff_dotplot_fixed) /
  (ORA_exp2_methdiff_dotplot_fixed +
...
combined_methdiff_dotplot
ggsave("ORA_both_experiments_methdiff_dotplot_fixed.png", combined_methdiff_dotplot, width = 24, height = 30, units= "cm", dpi = 600)
...
ORA_results_exp2_supplementary <- ORA_results_exp2_combined %>%
  mutate(
    ID = str_extract(as.character(ID), "GO:[0-9]{7}"),
    region_label = recode(
      region,
...
ORA_results_exp2_supplementary_clean <- ORA_results_exp2_supplementary %>%
  mutate(
...
write.csv(
  ORA_results_exp2_supplementary_clean,
  file = "Supplementary_Table_ORA_exp2_all_results.csv",
  row.names = FALSE
)

writexl::write_xlsx(
  ORA_results_exp2_supplementary_clean,
  path = "Supplementary_Table_ORA_exp2_all_results.xlsx"
)
