#!/usr/bin/env Rscript
#SBATCH --job-name=regionalised_dma_with_outliers
#SBATCH --mem=400G
...
figure_dir <- file.path(results_dir, "figures", "regionalised_dma_with_outliers")
...
loadings_dir <- file.path(results_dir, "tables", "regionalised_dma_with_outliers", "pca_loadings")

model_output_dir <- file.path(
  results_dir,
  "model_outputs",
  "regionalised_dma_with_outliers"
)

tmp_methylkit_dir <- file.path(
  results_dir,
  "tmp",
  "regionalised_dma_with_outliers_methylkit_inputs"
)
...
# -------------------------------
# Sample definitions: WITH outliers included
# -------------------------------
...
get_sample_info <- function(experiment) {
  if (experiment == "exp1") {
    data.frame(
      ...
      file_sample_id = c("3", "9", "13", "15", "16", "17", "22", "27"),
      methylkit_sample_id = c("3", "9", "13", "15", "16", "17", "22", "27"),
      treatment = c(1, 0, 0, 1, 0, 0, 1, 1),
      ind_id = factor(c("5", "15", "9", "15", "2", "5", "9", "2")),
      stringsAsFactors = FALSE
    )
    ...
    data.frame(
      ...
      file_sample_id = c("6", "7", "8", "10", "14", "20", "21", "29_11", "32_18", "36_2"),
      methylkit_sample_id = c("6", "7", "8", "10", "14", "20", "21", "11", "18", "2"),
      treatment = c(0, 0, 1, 0, 1, 1, 1, 0, 0, 1),
      ind_id = factor(c("19", "8", "20", "6", "8", "19", "12", "12", "20", "6")),
      stringsAsFactors = FALSE
    )
  } else {
...
# Input preparation
# -------------------------------
# The regionalisation script writes files with extra metadata columns.
# methylKit expects bismarkCoverage-like six-column files:
# chr, start, end, percent_methylation, numCs, numTs.
# This function converts each regionalised count file into a temporary
# methylKit-compatible file.
...
plot_regional_pca <- function(act_data_normed_fu, experiment, context, region, sample_metadata) {
  ...
  loadings_file <- file.path(
    ...
    context,
    safe_region,
    paste0("PCA_1-2_loadings_", context, "_", safe_region, "_", experiment, "_with_outliers.txt")
  )
  ...
  combined_pca_plot <- ggplot(pca_data, aes(x = PC1, y = PC2)) +
    ...
    theme_bw() +
    labs(
      title = paste("Regional PCA:", context, region, experiment, "with outliers"),
  ...
  pca_file <- file.path(
    ...
    context,
    safe_region,
    paste0("PCA_1-2_plot_", context, "_", safe_region, "_", experiment, "_with_outliers.png")
  )
  ...
  for (context in contexts) {
    for (region in regions) {
      ...
      if (conversion_failed || any(is.na(converted_files))) {
        message("Skipping ", experiment, " / ", context, " / ", region, " because input conversion failed.")
        next
      }
      ...
      tryCatch({
        qc_file <- file.path(
          ...
          context,
          safe_region,
          paste0("raw_methylation_plots_act_", context, "_", safe_region, "_", experiment, "_with_outliers.png")
        )
      ...
      tryCatch({
        coverage_file <- file.path(
          ...
          context,
          safe_region,
          paste0("raw_coverage_plots_act_", context, "_", safe_region, "_", experiment, "_with_outliers.png")
        )
      ...
      tryCatch({
        dmb_data_exp <- calculateDiffMeth(
          ...
          overdispersion = "MN",
          mc.cores = n_cores,
          suffix = paste0(context, "_fu3_", safe_region, "_odMNtestC_with_outliers")
        )
      }, error = function(e) {
      ...
      tryCatch({
        ...
        volcano_file <- file.path(
          ...
          context,
          safe_region,
          paste0("volcano_plot_", experiment, "_", context, "_", safe_region, "_with_outliers_regions.png")
        )
        ...
        rdata_file <- file.path(
          output_dir,
          paste0("Actinia_DMRs_d5_", experiment, "_", context, "_", safe_region, "_with_outliers_df_ann.RData")
        )

        txt_file <- file.path(
          output_dir,
          paste0("Actinia_DMRs_d5_", experiment, "_", context, "_", safe_region, "_with_outliers_df_ann.txt")
        )
...
message("All regionalised DMR analyses with outliers included are complete.")