...
# Load colData
colData <- read.table("C:/Users/r02hw22/OneDrive/Documents/PhD/PhD---omics-analysis-portfolio/Chapter 3/Data/MS_colData.txt", header = T, sep = '\t', 
...
## Plots ----
diesel_LRT_vs_Wald_LFC_shrunk <- ggplot(
  diesel_cor_df,
  aes(x = lfc_LRT_diesel_shrunk, y = lfc_Wald_diesel_shrunk)
) +
...
diesel_LRT_vs_Wald_LFC_raw <- ggplot(
  diesel_cor_df,
  aes(x = lfc_LRT_diesel_raw, y = lfc_Wald_diesel_raw)
) +
  geom_point(alpha = 0.4, size = 1.5) +
  labs(
...
salinity_LRT_vs_Wald_LFC_shrunk <- ggplot(
  salinity_cor_df,
  aes(x = lfc_LRT_salinity_shrunk, y = lfc_Wald_salinity_shrunk)
) +
  geom_point(alpha = 0.4, size = 1.5) +
  labs(
    x = "LRT log2FoldChange - shrunk",
...
salinity_LRT_vs_Wald_LFC_raw <- ggplot(
  salinity_cor_df,
  aes(x = lfc_LRT_salinity_raw, y = lfc_Wald_salinity_raw)
) +
  geom_point(alpha = 0.4, size = 1.5) +
  labs(
    x = "LRT log2FoldChange",
...
# save plot
ggsave("C:/Users/r02hw22/OneDrive/Documents/PhD/PhD---omics-analysis-portfolio/Chapter 3/Figures/Main Figures/LRT_and_Wald_LRT_correlation.png",  
       all_lfc_plots,
       width = 12, 
       height = 8)
